...
def parse_options(
    ...
    for full_phase, dataset in opt.datasets.items():
        ...
        if dataset.dataroot_ref is not None:
            if isinstance(dataset.dataroot_ref, str):
                dataset.dataroot_ref = [osp.expanduser(dataset.dataroot_ref)]
            else:
                dataset.dataroot_ref = [osp.expanduser(p) for p in dataset.dataroot_ref]
    ...
    if opt.path.pretrain_network_deg is not None:
        opt.path.pretrain_network_deg = osp.expanduser(opt.path.pretrain_network_deg)
    if opt.path.pretrain_network_d_lr is not None:
        opt.path.pretrain_network_d_lr = osp.expanduser(opt.path.pretrain_network_d_lr)
    if opt.path.pretrain_network_d_sr is not None:
        opt.path.pretrain_network_d_sr = osp.expanduser(opt.path.pretrain_network_d_sr)
...
def diff_user_vs_template(user_yaml_path: Path) -> tuple[str, str]:
    ...
    if template_str and template_filename:
        ...
        with open(user_yaml_path, encoding="utf-8") as f:
